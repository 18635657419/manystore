define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'stripestatistics/index' + location.search,
                    add_url: 'stripestatistics/add',
                    edit_url: 'stripestatistics/edit',
                    del_url: 'stripestatistics/del',
                    multi_url: 'stripestatistics/multi',
                    import_url: 'stripestatistics/import',
                    table: 'stripestatistics',
                }
            });
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        {field: 'stripe.email', title: __('邮箱'), operate: 'like',visible:false},
                        {field: 'stripe.email', title: __('stripe账号/收款周期（天）'),searchable:false,formatter:function (value,data) {
                            var html = '<a class="ppaccount" data-email='+value+' style="clore:red;cursor:pointer">'+value+' / '+data.period+'天</a>'+''
                        ...
                        {field: 'stripe.status', title: __('Status'), searchList: {"on":__('Status on'),"off":__('Status off'),"limited":__('Status limited'),"limited180":__('Status limited180'),"autooff":__('Status Offline')}, formatter: Table.api.formatter.status},
            ...
            $(document).on("click", ".ppaccount", function () {
                ...
                layer.open({
                    area:["90%","90%"],
                    type: 2, 
                    content: '/cnLyKYhOfC.php/stripe?email='+email+'&show_type=true' 
                }); 
