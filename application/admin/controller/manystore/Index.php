    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            ...
            $list = $this->model
                ...
                ->limit($offset, $limit)
                ->select();
            $PpaccountModel = new \app\admin\model\Ppaccount;
            $StripeModel = new \app\admin\model\Stripe;
            foreach($list as $key => $val){
                $ppaccount = $PpaccountModel->where('shop_id',$val['id'])->count();
                $stripecount = $StripeModel->where('shop_id',$val['id'])->count();

                $val['ppaccount'] = $ppaccount;
                $val['stripecount'] = $stripecount;
            }
